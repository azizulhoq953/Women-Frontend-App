...
const GetMentalHealthPosts = () => {
  ...
  useEffect(() => {
    const fetchMentalHealthPosts = async () => {
      ...
      try {
        const response = await fetch("https://women-backend-production.up.railway.app/api/admin/mental", {
          method: "GET",
          headers: {
  ...
  const handleRemovePost = async (postId) => {
    ...
    try {
      const response = await fetch(`https://women-backend-production.up.railway.app/api/admin/mental/${postId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
